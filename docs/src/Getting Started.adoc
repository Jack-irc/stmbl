...
For Windows only you will also need the STM Virtual Comport driver (So the board will show up as a DFU device) 
AND DfuSe USB device firmware upgrade STMicroelectronics extension (to actually send the .dfu file to the board)
...
### WARNING / CAUTION !!! ######################################################################################################
6. Saturation Error: saturation error means the output saturates, and it doesnt see any effect on the feedback.DO NOT INCREASE OR DISABLE THE SATURATION SET TIME !!! the output of any stage of any pid satureates for more than a set time
and ,changing or disabling this will not fix the issue, only burn the motor. its to protect the motor, drive and hardware.

In other words, it means that the STMBL drive is sending power to the motor and the motor is not moving it is either locked or the encoder feedback is failing so, the drive will stop sending power to the motor after the max stauration time in order to avoid burning the motor windings...etc.
################################################################################################################################
